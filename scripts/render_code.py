...
class BirimKesirDersi(Scene):
    def construct(self):
        ...
        lines = VGroup([Line(ORIGIN, [2np.cos(iTAU/4), 2np.sin(iTAU/4), 0], color=DARK_GRAY) for i in range(4)])
        self.play(Create(whole), Create(lines))
        self.wait(1)
        
        # AnnularSector kullanarak parametre hatasını kesin olarak çözüyoruz
        ...
        frac = MathTex(r'rac{1}{4}', color=BLUE, font_size=120).move_to(RIGHT*4)
